...
# --- Variables
GROUPS_TO_ADD="sudo,docker,admins,vboxsf"
...
if ! sudo -u "$USERNAME" HOME="$USER_HOME" yadm clone "$DOTFILES_URL"; then
    ...
    sudo -u "$USERNAME" HOME="$USER_HOME" yadm clone "$DOTFILES_URL"
fi

# 4) Forcer l'application des dotfiles depuis la branche main
sudo -u "$USERNAME" HOME="$USER_HOME" yadm reset --hard main
